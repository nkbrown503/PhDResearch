...
for k in range(len(Mesh_X)):
    ...
    filename = 'DDQN_TopOpt_Generalized_CNN_Elements_'+str(Mesh_X[k])+'by'+str(Mesh_Y[k])
    class TopOpt_Gen(Env):
        ...
        def step(self,action):
            ...
            if rs_place==1 and action not in self.BC and SingleCheck[1]==True:
                ...
                reward2=Reward_Ind[int(1-(np.reshape(self.Stress_state,((Elements_X+1)*(Elements_Y+1),1))[action])*1000)-1]
            
                reward=statistics.mean([reward,reward2])
                
                self.Stress_state=Run_Results[4]
                self.Stress_state=np.reshape(self.Stress_state,(Elements_X+1,Elements_Y+1))
                self.state=np.zeros((Elements_X+1,Elements_Y+1,3))
        ...
        def reset(self):
            ...
            self.Stress_state =FEA_SOLVER_GENERAL.FEASolve(VoidCheck,Lx,Ly,Elements_X,Elements_Y,self.Loaded_Node,self.Load_Direction,self.BC1,self.BC2,Stress=True)[4]
        ...
        def reset_conditions(self):
            ...
            while self.Max_SE_Tot<=0 or self.Max_SE_Tot>5000:
                self.BC1=int(random.choice([i for i in Go_List]))
                self.BC2=int(random.choice([i for i in Go_List]))
                BC_set=[self.BC1,self.BC2]
                self.Loaded_Node=int(random.choice([i for i in Go_List]))
                while self.Loaded_Node in BC_set:
                    self.Loaded_Node=int(random.choice([i for i in Go_List]))
                ...
                self.LC_state=list(np.zeros((1,(Elements_X+1)*(Elements_Y+1)))[0])
                self.LC_state[self.Loaded_Node]=1
                self.LC_state=np.reshape(self.LC_state,(Elements_X+1,Elements_Y+1))
                ...
                self.BC_state=list(np.zeros((1,(Elements_X+1)*(Elements_Y+1)))[0])
                self.BC_state[self.BC1]=1
                self.BC_state[self.BC2]=1
                self.BC_state=np.reshape(self.BC_state,(Elements_X+1,Elements_Y+1))
    ...
    class DuelingDeepQNetwork(keras.Model):
        def __init__(self, n_actions):
            ...
            self.model.add(layers.Conv2D(1,(1,1),activation='relu',input_shape=(Elements_X+1,Elements_Y+1,3)))
            ...
            #self.model.add(layers.Dense(36,activation='relu'))
            #self.model.add(layers.Dense(36,activation='relu'))
            #self.model.add(layers.Dense(36,activation='relu'))
    ...
    class Agent():
        def __init__(self, lr, gamma, n_actions, epsilon, batch_size,
            ...
            self.q_eval = DuelingDeepQNetwork((Elements_X+1)*(Elements_Y+1))
            self.q_next = DuelingDeepQNetwork((Elements_X+1)*(Elements_Y+1))
        ...
        def choose_action(self, observation,load_checkpoint,Testing):
            if np.random.random() < self.epsilon and load_checkpoint is False and not Testing:
                action = np.random.choice(self.action_space)
            else:

                state = observation
                state=state.reshape(-1,Elements_X+1,Elements_Y+1,3)
        ...
        def learn(self):
            ...
            Loss=self.q_eval.fit(states,q_target,batch_size=self.batch_size,epochs=1,verbose=0)
    ...
    agent = Agent(lr=5e-3, gamma=0,filename=filename, n_actions=env.action_space.n, epsilon=1.0,
                      batch_size=128, input_dims=[Elements_X+1,Elements_Y+1,3])
    n_games = 100_000
    ...
    for i in range(n_games):
        ...
        if load_checkpoint:
            ...
            env.BC_state=list(np.zeros((1,(Elements_X+1)*(Elements_Y+1)))[0])
            env.BC_state[env.BC1]=1
            env.BC_state[env.BC2]=1
            env.BC_state=np.reshape(env.BC_state,(Elements_X+1,Elements_Y+1))
            ...
            env.LC_state=list(np.zeros((1,(Elements_X+1)*(Elements_Y+1)))[0])
            env.LC_state[env.Loaded_Node-NodeSize]=1
            env.LC_state=np.reshape(env.LC_state,(Elements_X+1,Elements_Y+1))
        ...
        while not done:
            ...
            observation_, reward, done, It,info = env.step(action)
        ...
        if i>50:
            ...
            if Testing is True and score>=best_test_score and i>1:
            ...
            #if avg_score >= best_score and not load_checkpoint and i>200:
                #best_score = avg_score
