...
def Condition_Transform(Lx,Ly,Old_EX,Old_EY,New_EX,New_EY,BC1,BC2,BC1_E,BC2_E,LC,Load_Type,Load_Direction):
    ...
    New_LC1,New_LC2=LC_Nodes(New_LC_E,Load_Type,Lx,Ly,Old_EX,Old_EY,Node_Location=False)
    New_BC1,New_BC2=BC_Nodes(New_BC1_E,Lx,Ly,Old_EX,Old_EY)
    New_BC3,New_BC4=BC_Nodes(New_BC2_E,Lx,Ly,Old_EX,Old_EY)
    Row_BC1=math.floor(New_BC1_E/Old_EX)
    Col_BC1=int(round(math.modf(New_BC1_E/Old_EX)[0]*Old_EX,0))
    Row_BC2=math.floor(New_BC2_E/Old_EX)
    Col_BC2=int(round(math.modf(New_BC2_E/Old_EX)[0]*Old_EX,0))
    Row_LC=math.floor(New_LC_E/Old_EX)
    Col_LC=int(round(math.modf(New_LC_E/Old_EX)[0]*Old_EX,0))
    BC2_BC1=abs(Row_BC2-Row_BC1)+abs(Col_BC2-Col_BC1)
    BC2_LC=abs(Row_BC2-Row_LC)+abs(Col_BC2-Col_LC)
    BC1_LC=abs(Row_BC1-Row_LC)+abs(Col_BC1-Col_LC)
    Len_Mat=[BC2_BC1,BC2_LC,BC1_LC]
    Len_Mat.remove(max(Len_Mat))
    Min_Length=sum(Len_Mat)+1
  

    return New_BC1, New_BC2,New_BC3,New_BC4,New_BC1_E,New_BC2_E,New_LC_E,New_LC1,New_LC2,Min_Length,Load_Direction
