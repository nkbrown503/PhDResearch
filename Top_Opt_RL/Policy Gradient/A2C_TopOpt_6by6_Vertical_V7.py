...
Max_VM_Tot=np.max((FEA_SOLVER_V.FEASolve(VoidCheck,Lx,Ly,Elements_X,Elements_Y,Stress=True)[2]))
    ...
    def step(self,action, New,preset_reward,preset_state):
        ...
        if rs_place==1 and action not in LeftBC and SingleCheck[1]==True:
            ...
            if New is True:
                ...
                Max_VM_Ep=np.max(Run_Results[2])
                self.state=Run_Results[3]
                reward=(Max_VM_Tot/Max_VM_Ep)*It
            else:
...
class ActorCriticNetwork(keras.Model):
    def __init__(self, n_actions, fc1_dims=256, fc2_dims=256, fc3_dims=256, fc4_dims=256,
            name='actor_critic_6b6_V_V6', chkpt_dir='tmp/actor_critic'):
...
class Agent:
    def __init__(self, n_actions,alpha=0.001, gamma=0.99, ):
        self.gamma = gamma
        self.n_actions = n_actions
        self.action = None
    ...
    def choose_action(self, observation):
        ...
        if action not in range(0,ElementSize):
...
agent = Agent(n_actions=env.action_space.n, alpha=1e-4, )
n_games = 50_000
# uncomment this line and do a mkdir tmp && mkdir video if you want to
# record video of the agent playing the game.
#env = wrappers.Monitor(env, 'tmp/video', video_callable=lambda episode_id: True, force=True)
filename = 'TopOpt_6by6_V_V7'
