...
class TopOpt_6by6_V(Env):
    ...
    def step(self,action, New,preset_reward):
        #Apply Action
        ...
        if rs_place==1 and action not in LeftBC and SingleCheck[1]==True:
            ...
            if New is True:
                Max_VM_Ep=np.max((FEA_SOLVER_V.FEASolve(list(self.state),Lx,Ly,Elements_X,Elements_Y,Stress=True)[2]))
...
for i in range(n_games):
    ...
    while not done:
        ...
        if len(Data_Loc[0])==0:
            ...
            New=False
        observation_, reward, done, info = env.step(action,New,preset_reward)
